...
COVERAGE_INIT = r"""
testCoverage = m.global.testCoverage
testCoverage["{}"] = CreateObject("roArray", {}, false)
m.global.testCoverage = testCoverage"""

# MARK_TEST_FUNC = """sub {0}_markTestCoverage(index)
#   componentCoverage = m.global.testCoverage["{0}"]
#   if componentCoverage = invalid
#     testCoverage = m.global.testCoverage
#     testCoverage["{0}"] = createObject("roSGNode", "ContentNode")
#     testCoverage["{0}"].addFields({{"length":{1}}})
#     m.global.testCoverage = testCoverage
#     componentCoverage = m.global.testCoverage["{0}"]
#   end if
#   index = str(index)
#   m.global.testCoverage["{0}"].addFields({{index:true}})
# end sub

# """
# MARK_TEST_FUNC = """sub {0}_markTestCoverage(index)
#   'if m.{0}Coverage = invalid
#     'm.{0}Coverage = m.global.testCoverage.findNode("{0}")
#   'end if
#   if m.{0}Coverage = invalid
#     m.{0}Coverage = m.global.testCoverage.createChild("ContentNode")
#     m.{0}Coverage.id = "{0}"
#     m.{0}Coverage.addFields({{"length":{1}}})
#   end if
#   index = str(index)
#   fields = {{}}
#   fields[index] = true
#   m.{0}Coverage.addFields(fields)
# end sub
...
REPORT_COVERAGE_LINES = r"""    totalLinesCount = 0
    ...
    for each componentName in m.global.testCoverage.getFields()
      if componentName <> "change" and componentName <> "focusable" and componentName <> "focusedChild" and componentName <> "id"
...
def transform_component(component):
    ...
    # init_out = mark_function + init_function[:insert_index] + coverage_init + init_function[insert_index:]
